...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.drinksTable deselectRowAtIndexPath:[self.drinksTable indexPathForSelectedRow] animated:YES];
}
...
- (void)getDinnerItems {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/getDrinks.php", root]];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        [self.drinksTable setNeedsDisplay]; 
        [HUD hide:TRUE];
...
- (void)viewDidLoad
{
    ...
    if (_refreshHeaderView == nil) {
		...
		EGORefreshTableHeaderView *view = [[EGORefreshTableHeaderView alloc] initWithFrame:CGRectMake(0.0f, 0.0f - self.drinksTable.bounds.size.height, self.view.frame.size.width, self.drinksTable.bounds.size.height)];
		view.delegate = self;
		[self.drinksTable addSubview:view];
    ...
    sBar.delegate = self;
    drinksTable.tableHeaderView = sBar;
    ...
    searchController.searchResultsTableView.rowHeight = drinksTable.rowHeight;
...
- (void)doneLoadingTableViewData{
	...
	_reloading = NO;
	[_refreshHeaderView egoRefreshScrollViewDataSourceDidFinishedLoading:self.drinksTable];
}
