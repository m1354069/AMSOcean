...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.dessertTable deselectRowAtIndexPath:[self.dessertTable indexPathForSelectedRow] animated:YES];
}
...
- (void)getDinnerItems {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/getDessert.php", root]];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        [self.dessertTable reloadData];
        [self.dessertTable setNeedsDisplay]; 
        [HUD hide:TRUE];
...
- (void)viewDidLoad
{
    ...
    if (_refreshHeaderView == nil) {
		...
		EGORefreshTableHeaderView *view = [[EGORefreshTableHeaderView alloc] initWithFrame:CGRectMake(0.0f, 0.0f - self.dessertTable.bounds.size.height, self.view.frame.size.width, self.dessertTable.bounds.size.height)];
		view.delegate = self;
		[self.dessertTable addSubview:view];
    ...
    sBar.delegate = self;
    dessertTable.tableHeaderView = sBar;
    ...
    searchController.searchResultsTableView.rowHeight = dessertTable.rowHeight;
...
- (void)doneLoadingTableViewData{
	...
	_reloading = NO;
	[_refreshHeaderView egoRefreshScrollViewDataSourceDidFinishedLoading:self.dessertTable];
}
